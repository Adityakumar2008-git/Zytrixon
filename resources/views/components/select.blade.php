...
<div class="space-y-2">
    ...
    <select
        ...
        {{ $attributes->merge([
            'class' => 'w-full rounded-xl border bg-white px-4 py-3 text-sm text-neutral-900 transition-colors focus:border-neutral-900 focus:outline-none focus:ring-1 focus:ring-neutral-900 disabled:opacity-50 ' . ($error || $errors->has($name) ? 'border-red-500' : 'border-neutral-300')
        ]) }}
    >
        @if ($placeholder)
