<x-layouts.app
    ...
    <x-section id="sectors" class="bg-[#F7F7F8] dark:bg-[#0E0F12]">
        <div>
        ...
        <div class="mt-12 grid gap-6 sm:grid-cols-2 lg:grid-cols-5">
            @foreach ($industries as $industry)
                <div class="rounded-2xl border border-neutral-200/80 dark:border-neutral-800/80 bg-white dark:bg-[#141519] p-6 transition-all duration-300 hover:shadow-lg hover:border-neutral-300 dark:hover:border-neutral-700 flex flex-col justify-between">
                    <div>
                        <span class="font-mono text-xs uppercase tracking-[0.2em] text-neutral-400">{{ $industry['slug'] }}</span>
                        <h3 class="mt-3 text-lg font-bold text-neutral-900 dark:text-white">{{ $industry['title'] }}</h3>
                        <p class="mt-2 text-xs leading-relaxed text-neutral-600 dark:text-neutral-400">{{ $industry['description'] }}</p>
                    </div>
                </div>
            @endforeach
        </div>
